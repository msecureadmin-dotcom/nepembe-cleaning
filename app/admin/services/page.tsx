  ...
  return (
    <div>
      ...
      <div className="grid gap-4 max-w-xl">
        {services.map((s, i) => (
          <div key={s.id} className="bg-white rounded-2xl border border-[#eadbc2] p-4 flex items-center gap-4">
            ...
            <button onClick={() => setConfirmItem({ id: s.id, name: s.title })} className="bg-red-50 text-red-700 font-bold px-3 py-1 rounded-xl text-xs">Delete</button>
          </div>
        ))}
      ...
      <ConfirmModal
        ...
        onConfirm={() => { if (confirmItem) deleteService(confirmItem.id); setConfirmItem(null); }}
